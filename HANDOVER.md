...
16. (Thomas) Archive images must CHANGE, not just zoom: every image appears once (one ~6-7 s move: push-in or pull-back, alternating), then cut to the next image. assemble.py now does one shot per image, so each slot needs ceil(duration / 7) distinct images. Slots longer than ~25 s should become maps instead (Thomas: the Cracker Line and the Nashville telegrams/ice storm were turned from archive slots into map paragraphs; retag the paragraph `[MAP: id-b | ...]` in script.md and audio/timing.json, no re-recording needed).
**Lessons from the Thomas video (2026-09-26):**
17. Wikimedia's API rate-limits (429) the cloud IP after ~1 h of image fetching. The Library of Congress JSON API (`https://www.loc.gov/item/<id>/?fo=json`, images on tile.loc.gov) is not limited and holds most Civil War photos/lithographs. Tell the image agent to use LoC first for 19th-century American subjects.
18. `tools/water.py BASEMAP LEVEL_M` traces rivers/lakes from the elevation tiles (level = river surface in m, see the 1st percentile of the elevation) → assets/BASEMAP_water.png. Works well on dammed rivers (Tennessee at Chattanooga), badly on free-flowing ones (Cumberland at Nashville): check the overlay before using it.
19. Sonnet regional maps came out too small (9-12 px labels, camera scale < 1, portraits left as plaques because they were built before the images existed) and with placement errors (Franklin line on the wrong side of town). Put in the brief: labels >= 20 px on screen, camera >= 1.2 on anything smaller than a state, and re-check `ls assets/media` right before the final build. Review their contact sheets yourself.
20. The account's weekly limit can stop all agents mid-run; they restart cleanly (resend the brief, they reuse existing scene files).
21. Sentence-split timing: abbreviations with a period ("Ulysses S. Grant", "U.S.", "Robert E. Lee") split sentences in narrate.py. Write them without the period in the script.
**Research accuracy:**
